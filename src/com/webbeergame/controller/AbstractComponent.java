...
public abstract class AbstractComponent implements Component, Serializable{
	...
	public void setNewOrder(Integer order) {
		log.log(Level.INFO,"Setting {0} order to Player "+ this.player.getName(), order);
		this.facingOrder = order;
		Integer totalOrder = order+ this.backOrder;
		if(this.getInventory() > totalOrder){
			this.setSupplied(totalOrder);
		} else {
			this.setSupplied(getInventory());
		}

		log.log(Level.INFO,"Player {0} supplied "+ this.supplied, this.player.getName());
	}
}
